...
GPT4ALL_MODELS_ENABLED = (
    os.environ["GPT4ALL_MODELS_ENABLED"]
    if "GPT4ALL_MODELS_ENABLED" in os.environ
    else True
)
...
async def assist(item: AssistItem):
    with SessionLocal() as db:
        ...
        if item.version.startswith("gpt-"):
            ...
            raise HTTPException(
                status_code=400, detail="Invalid version: " + item.version
            )
...
async def get_history(uid: str):
    with SessionLocal() as db:
        ...
        browser_history = (
            db.query(BrowserHistory).filter(BrowserHistory.uid == uid).all()
        )
